...
if ($stmt = $conn->prepare("SELECT content, username FROM comments WHERE id=? and is_deleted=0")) {
...
<body>
  ...
  <main class="board">
    ...
    <form class="board__form" method="POST" action="handle_update_comment.php">
      <!-- 抓留言內容 -->
      <?php if (get_permission_edit($username, $comment_username)) { ?>
