...
// [ 功能 ] 根據點選的遊戲名稱產生串流清單跟 load more 按鈕
document.querySelector('.nav__menu').addEventListener('click', (e) => {
  if (e.target.classList.contains('nav__menu__selection')) {
      ...
      if (request.status >= 200 && request.status < 300) {
        ...
        try {
          ...
          for (let i = 0; i < json.streams.length; i++) {
            infoObjs[i] = {
              frame: json.streams[i].preview.large,
              avatar: json.streams[i].channel.logo,
              title: json.streams[i].channel.status,
              author: json.streams[i].channel.name,
    ...
    // 加入 request header，必須在 open() 之前 send() 之後
    request.setRequestHeader('Accept', 'application/vnd.twitchtv.v5+json')
    request.setRequestHeader('Client-ID', 'c1yk5e1n85nan22a60u0qzvjyu2rpf')
    request.send()
  }
})
...
// 點擊 load more 按鈕，載入 20 個接續的 live
