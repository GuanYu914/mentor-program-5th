...
const clientId = 'c1yk5e1n85nan22a60u0qzvjyu2rpf'
const clientSecret = 'x0emovg8l6nr8olye7nelygchbkoen' // 用來存取需要 OAuth 的 API
const urlGetAPPAccessToken = `https://id.twitch.tv/oauth2/token?client_id=${clientId}&client_secret=${clientSecret}&grant_type=client_credentials`
      ...
      request(
        {
          ...
          headers: {                                                  // 添加使用舊版 Twitch API 的規範 Header
            ...
            'Client-Id': `${clientId}`,                               // 申請使用 Twitch api 的 Client ID
            ...
            request(
              ...
              function(error, httpResponse, body) {                   // 根據遊戲 ID 列出串流列表
                if (!error && httpResponse.statusCode >= 200 && httpResponse.statusCode < 300) {
                  ...
                  let nxtPageCursor = streamInfo.pagination.cursor                // 拿到下一頁指標
                  // console.log(`first fetch: ${nxtPageCursor}`);
                  ...
                  while (cnt) {                                                   // 重複一次，每次印出 100 筆資料，一共為 100 筆資料
                    ...
                    urlGetStreamList += `first=100&after=${nxtPageCursor}`
                    ...
                    request(
                      ...
                      function(error, httpResponse, body) {
                        if (!error && httpResponse.statusCode >= 200 && httpResponse.statusCode < 300) {
                          ...
                          // console.log(`current cursor: ${nxtPageCursor}`)
                          nxtPageCursor = tmp.pagination.cursor
                          // console.log(`next cursor: ${nxtPageCursor}`)
                        } else {
